package ro.ciacob.utils {
    ...
    public final class Arrays {
         ...
         */
        public static function getSubsetOf(collection:Array, size:uint, uniqueElements:Boolean = true, workOnSource:Boolean = false):Array {
        ...
        public static function removeOneDupplicate(array:Array, beginFromEnd:Boolean = false):Boolean {
            ...
            if (beginFromEnd) {
                outerLoopA: for (valueIndex = array.length - 1; valueIndex >= 0; valueIndex--) {
                    searchValue = array[valueIndex];
                    innerLoopA: for (searchIndex = array.length - 1; searchIndex >= 0; searchIndex--) {
                ...
                outerLoopB: for (valueIndex = 0; valueIndex < array.length; valueIndex++) {
                    searchValue = array[valueIndex];
                    innerLoopB: for (searchIndex = 0; searchIndex < array.length; searchIndex++) {
                        if (array[searchIndex] === searchValue) {
                            if (searchIndex == valueIndex) {
                                continue;
                            }
        ...
        /**
         * Removes duplicate elements within a given array. Modifies the array in place, and does not
         * sort it. The equality test employed for primitives is `==`.
         *
         * @param	array The array to remove duplicate from.
         */
            ...
            if (array != null && array.length > 0) {
                ...
                for (var i:int = 0; i < tmp.length; i++) {
                    ...
                    if (el.constructor && el.constructor == Object) {
                        var isDuplicate:Boolean = objIndices.some(function(objIndex:int, index:int, arr:Array):Boolean {
                            var objToTest:Object = array[objIndex];
                            return Objects.compareObjects (el, objToTest);
                        });
        ...
        public static function intersect(a:Array, b:Array, noDupplicates:Boolean):Array {
            ...
            if (b.length > a.length)
                t = b, b = a, a = t; // swap to make sure "indexOf" runs over the shorter Array
         ...
         *
         * 			// Matching mode flags
         * 			Arrays.FILTER_ANY - an item is included if either of the keys in `condition` has a match;
         * 			Arrays.FILTER_ALL - an item is included only if all of the keys in `condition` have matches (default);
         * 			Arrays.FILTER_NONE - an item is included only if none of the keys in `condition` have a match;
         *
         * 			// Misc. Flags
         ...
         *
         * @return	A new, possibly empty Array, containing the matching items.
         */
