package ro.ciacob.utils
{
	...
	public final class Files
	{
		...
		private static const AUTO_RENAME_SCHEME_WITHOUT_EXTENSION:String='%s(%d)';
		private static const AUTO_RENAME_SCHEME_WITH_EXTENSION:String='%s(%d).%s';
		...
		*/
		private static function recursivelyListDirectory(directory:File, storage:Array, includeDirs:Boolean=true):void
		{
			if (directory.isDirectory)
			{
				...
				for (var i:int=0; i < files.length; i++)
				{
					if (files[i].isDirectory)
					{
						...
						recursivelyListDirectory(files[i], storage, includeDirs);
					}
					else
					{
						storage.push(files[i]);
		...
		/**
		 * Produces a Vector of File objects that were produced by recursivelly iterating over all items in the
		 * given directory.
		 *
		 ...
		 */
		public static function getDirContent(directory:File, includeDirs:Boolean=true):Vector.<File>
		{
			var storage:Array=[];
			recursivelyListDirectory(directory, storage, includeDirs);
